...
# [HACK] Hardcoding the legacy _expand_mask function to bypass transformers v4.32+ version errors
...
class gloss_free_model(nn.Module):
    def __init__(self, config, args, embed_dim=1024, pretrain=None):
        ...
        # 1. COMMENT OUT THE OLD PIXEL EXTRACTOR
        # self.backbone = FeatureExtracter(frozen=_('freeze_backbone', False))
        
        # 2. ADD YOUR NEW SKELETON PROJECTOR RIGHT HERE
    ...
    def share_forward(self, src_input):
        ...
        keypoints = src_input['input_ids'].cuda() 
        frames_feature = self.keypoint_projector(keypoints)
        attention_mask = src_input['attention_mask']
